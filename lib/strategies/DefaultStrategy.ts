...
export class DefaultStrategy extends BaseStrategy {
    build(prompt: PromptJson, options?: PromptTextOptions): string {
        ...
        const intention = prompt.custom_base_prompt ?? DEFAULT_INTENTION;
        ...
        const focusSentence = Array.isArray(aesthetic.material_fidelity) && aesthetic.material_fidelity.length > 0
            ? `Focus & Fidelity: ${aesthetic.material_fidelity.join("; ")}`
            : styleFallback.focus;
